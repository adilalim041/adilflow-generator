const TRUSTED_LOGO_CANDIDATES = {
    ...
    anthropic: simpleIconCandidates('anthropic'),
    google: simpleIconCandidates('google'),
    apple: simpleIconCandidates('apple'),
    meta: simpleIconCandidates('meta')
};
...
function simpleIconCandidates(slug) {
    return [
        simpleIconCandidate(slug),
        {
            url: `https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/${slug}.svg`,
            source_name: 'Simple Icons via jsDelivr',
...
function scoreEntityInField(value, entity, weight) {
    const text = normalizeText(value);
    if (!text) return 0;
    const aliases = [entity.name, entity.slug, ...(entity.aliases || [])].filter(Boolean);
    return aliases.reduce((sum, alias) => {
        if (!aliasMatches(text, alias)) return sum;
        return sum + Math.max(1, String(alias).length) * weight;
    }, 0);
}

function selectMentionedEntity(article, entities) {
    const briefCompany = String(article?.scores_detail?.article_brief?.entities?.main_company || '').trim().toLowerCase();
    if (briefCompany) {
        const briefMatch = (entities || []).find(entity => {
            if (entity?.entity_type !== 'company') return false;
            const names = [entity.name, entity.slug, ...(entity.aliases || [])]
                .filter(Boolean)
                .map(item => String(item).trim().toLowerCase());
            return names.includes(briefCompany);
        });
        if (briefMatch) return briefMatch;
    }

    const fields = [
        { value: article?.raw_title, weight: 80 },
        { value: article?.title, weight: 80 },
        { value: article?.headline, weight: 60 },
        { value: article?.url, weight: 35 },
        { value: article?.raw_summary, weight: 15 },
        { value: article?.summary, weight: 15 },
        { value: article?.raw_text, weight: 2 },
        { value: article?.body, weight: 2 }
    ];
    ...
    const candidates = (entities || [])
        ...
        .map(entity => ({
            entity,
            score: fields.reduce((sum, field) => sum + scoreEntityInField(field.value, entity, field.weight), 0)
        }))
...
function getTrustedLogoCandidates(entitySlug) {
    const slug = String(entitySlug || '').trim().toLowerCase();
    if (TRUSTED_LOGO_CANDIDATES[slug]) return TRUSTED_LOGO_CANDIDATES[slug];
    if (!/^[a-z0-9][a-z0-9-]{1,60}$/.test(slug)) return [];
    return simpleIconCandidates(slug).map(candidate => ({
        ...candidate,
        quality_score: Math.min(candidate.quality_score, 68),
        metadata_note: 'generic_simple_icons_slug_candidate'
    }));
}
...
async function resolveEntityLogoAsset({
    ...
    try {
        ...
        const candidates = getTrustedLogoCandidates(entity.slug);
        ...
        for (const nextCandidate of candidates) {
            try {
                downloaded = await downloadLogoCandidate(nextCandidate, fetchImpl);
                ...
                logger.warn?.({
                    ...
                    candidateUrl: nextCandidate.url,
                    error: candidateError.message
                }, 'Logo candidate failed');
            }
        }

        if (!candidate || !downloaded) {
            if (cachedAssets.legacy) {
                return { entity, asset: cachedAssets.legacy, source: 'cache_legacy_after_candidate_error' };
            }
            throw lastCandidateError || new Error('No logo candidate could be downloaded');
        }
