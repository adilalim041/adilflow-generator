...
const EDITORIAL_SCENE_DIRECTIVES = [
    ...
    },
    {
        ...
        aliases: [
            ...
            'senate', 'hearing', 'washington', 'order', 'usa', 'u s', 'us',
            'export', 'exports', 'export restriction', 'export restrictions',
            'restriction', 'restrictions', 'keep online', 'kept online',
            'forced online', 'online', 'g7', 'france'
        ],
        directive:
            'Storyboard: high-stakes satirical political editorial composite. Make the government or Donald Trump the visible pressure force, and make the AI company/public figure look constrained by that pressure. Use scenes like a White House hallway, courtroom, marble corridor, Oval Office desk, symbolic jail bars, a locked model vault, a giant government stamp, guards, export-control paperwork, or a looming government hand closing a cage around the model. If Donald Trump or a US agency is central in the article, include them only as an editorial metaphor and not as a claim that the exact scene happened. Make it dynamic, cinematic, provocative, and readable at thumbnail size.'
    },
    {
...
function findEntityVisualDirective(article) {
    if (article?._visualDirectiveOverride?.slug && article?._visualDirectiveOverride?.person) {
        return article._visualDirectiveOverride;
    }
...
function findEditorialSceneDirective(article) {
    const candidates = EDITORIAL_SCENE_DIRECTIVES
        ...
        .map(item => ({
            ...item,
            adjustedScore: item.score + (item.rule.priorityBoost || 0)
        }))
